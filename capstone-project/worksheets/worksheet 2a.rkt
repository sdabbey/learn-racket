...
;;;; -----------------------------------------------------------------------------
;;;; Note: this is Worksheet 2a. It's the same as the solved version of
;;;; Worksheet 1 until the lines "Worksheet 2a Begin" below.
;;;; -----------------------------------------------------------------------------
...
(define example-ship-5 (make-ship (make-posn 100 150) 0 (make-posn 0 0) ship-wedge))
(define example-ship-6 (make-ship (make-posn 250 300) 0 (make-posn 0 0) ship-wedge))
...
(check-expect
  (sw-render.v3 (make-sw example-ship-1 example-ship-2))
  (draw-ship (sw-wedge (make-sw example-ship-1 example-ship-2))
             (draw-ship (sw-needle (make-sw example-ship-1 example-ship-2))
                        starfield)))

(check-expect
  (sw-render.v3 (make-sw example-ship-1 example-ship-2))
  (draw-ship example-ship-1
             (draw-ship example-ship-2 starfield)))
...
;;;
;;; Move both ships simultaneously

(check-expect
  (move-ships.v3 (make-sw example-ship-1 example-ship-2))
  (make-sw (move-ship example-ship-1)
           (move-ship example-ship-2)))
...
;;;; -----------------------------------------------------------------------------
;;;; Worksheet 2a Begin: Adding rotation
;;;; -----------------------------------------------------------------------------

;; Rotation constant
(define rot 5)

;;; rotate-ship-counterclockwise : Ship -> Ship
;;; template (counterclockwise adds to the rotation):
;;; (define rotate-ship-counterclockwise ship
;;;   (update-rotation (modulo (+ ...))))
;;;
;;; Update the rotation of the ship in the counterclockwise position

;; Simple non-wrap case
(check-expect
  (rotate-ship-counterclockwise example-ship-1)
  (make-ship (ship-location example-ship-1)
             5
             (ship-velocity example-ship-1)
             (ship-image example-ship-1)))

;; Wrap-around case: 358 + 5 = 363 → 3 (mod 360)
(check-expect
  (rotate-ship-counterclockwise
    (make-ship (make-posn 100 150) 358 (make-posn 0 0) ship-wedge))
  (make-ship (make-posn 100 150) 3 (make-posn 0 0) ship-wedge))

(define (rotate-ship-counterclockwise ship)
  (update-rotation ship (modulo (+ (ship-rotation ship) rot) 360)))

;;; rotate-ship-clockwise : Ship -> Ship
;;; template (clockwise subtracts form the rotation):
;;; (define rotate-ship-clockwise ship
;;;   (update-rotation (modulo (- ...))))
;;;
;;; Update the rotation of the ship in the clockwise position

;; Simple non-wrap case: 10 − 5 = 5
(check-expect
  (rotate-ship-clockwise
    (make-ship (make-posn 200 250) 10 (make-posn 0 0) ship-wedge))
  (make-ship (make-posn 200 250) 5 (make-posn 0 0) ship-wedge))

;; Wrap-around case: 0 − 5 = −5 → 355 (mod 360)
(check-expect
  (rotate-ship-clockwise example-ship-1)
  (make-ship (ship-location example-ship-1)
             355
             (ship-velocity example-ship-1)
             (ship-image example-ship-1)))
...
;;; Write a key-event handler to rotate the ship according to keyboard input.
;;; Wire it up to big-bang
(define (key-event sw ke)
  (cond
    [(key=? "left" ke) (make-sw (rotate-ship-counterclockwise (sw-wedge sw)) (sw-needle sw))]
    [(key=? "right" ke) (make-sw (rotate-ship-clockwise (sw-wedge sw)) (sw-needle sw))]
    [(key=? "o" ke) (make-sw (sw-wedge sw) (rotate-ship-counterclockwise (sw-needle sw)))]
    [(key=? "u" ke) (make-sw (sw-wedge sw) (rotate-ship-clockwise (sw-needle sw)))]
    [else sw]))
...
;;; Make it go
(define (play x)
  (go.v4 (make-sw example-ship-5 example-ship-6)))

;;; There are a few problems here.  First, you can't move both ships simultaneously.  Key repeat
;;; prevents that.  Second, there are problems with the rotation (the ship turns unevenly).

;;; Spacewar-2b addresses these issues
;;;
;;; What needs changing:
;;; - To solve both ships moving simultaneously
;;;   - Add a turn parameter to:
;;;     - Example ships (including ship-*-wrap)
;;;     - Updaters as necessary
;;;   - Create a new updater and test
;;;     - update-turn
;;;   - Turn represent intent
;;;   - on-release cancels intent
;;; - To solve the ship rotation visual
;;;   - make the image centre the rotation centre
